				...
				switch (key) {
					case "login":
						...
						if (!await FS.fileExists(envFilePath)) {
							...
							return console.log((description || Message.runRequiresLogin).red)
						}
						// check login token age
						let env = require(envFilePath)
						if (env.lastLogin < Date.now() - cfg.tokenTTL) {
							// env token is too old
							return console.log(Message.oldToken.red)
						}
						break
					case "oauth":
