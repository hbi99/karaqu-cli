...
module.exports = async (src, dest) => {
	/*
		aliases: --build, -b
		description: Builds your source folder to an executable app in Karaqu.
	*/

	const { FS, Message } = require("./common");
	const { Build } = require("karaqu-builder");
	...
	Build({ source, destination, uglify, compress })
		.then(build => {
			console.log(`Built Karaqu Application ${build.name.white} v${build.version}`);
